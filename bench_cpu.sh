...
RUSTFLAGS="-C target-cpu=native" RUST_LOG=info /usr/bin/time -v /root/.cargo/bin/cargo run --release -p sp1-host-cpu -- --n-signatures 1 --max-segment-limit 18 execute 2>&1 | tee -a /workspace/cpu/sp1_1_sig.txt
